(function () {/**
   ...
   * @param wait {number=}
   * @returns {function(...[*]=)}
   */function a(a, b = 100) { let c; return function (...d) { clearTimeout(c), c = setTimeout(() => { a.apply(this, d) }, b) } } function b(a) { a || (a = window.location.search); const b = a.substr(1), c = {}; return !b.length || b.split("&").forEach(a => { const b = a.split("="); c[b[0]] = decodeURIComponent(b[1]) }), c } function c() {
      let a = new Date().getTime(), b = performance && performance.now && 1e3 * performance.now() || 0; return "xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(/[xy]/g, d => {
         var c = Math.floor; let e = 16 * Math.random();// eslint-disable-next-line no-bitwise
         return 0 < a ? (e = 0 | (a + e) % 16, a = c(a / 16)) : (e = 0 | (b + e) % 16, b = c(b / 16)), ("x" === d ? e : 8 | 3 & e).toString(16)
      })
   } const d = new function () {
      const d = window.API_URL || "http://localhost:3000", e = ["utm_source", "utm_medium", "utm_campaign", "utm_term", "utm_content"], f = "__site-stat__";// 30 mins according to Google metrics
/**
